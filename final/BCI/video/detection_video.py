...
weights_path = os.path.join(models_dir, 'init_weights\mask_rcnn_alcohol_0344.h5')
...
def learning(v):
    ...
    # DB save variable
    global count_obj  # ratio
    global total_obj  # total_ratio
    global obj_sec
    ...
    print("재생시간 잘 들어오니? ", play_time)
    gugan = 100 / play_time  # 구간
    gugan = round(gugan, 2)  # 구간 소숫점 두자리까지
    ...
    for ob, sec3 in obj_sec.items():
        sec=[]
        for j in range(0,len(sec3)):
            sec.append(sec3[j])
        print("db 에 저장될 timeline: ",sec3,"복사한: ", sec)
        print(ob)
        sec2=[]
        for i in range(0, len(sec3)):
            sec[i] = sec[i] - 1  # 1씩 빼줘

        for i in range(0, len(sec3)):
            sec[i] = round(sec[i] * gugan * 1.0, 2)  # 시작점 setting
            sec2.append(str(round(sec[i] + gugan, 2)) + "%")  # 끝 점 setting
            sec[i] = str(sec[i]) + "%"

        if ob =='alcohol':
            for i in range(0, len(sec)):
                global alcohol_dict
                alcohol_dict[sec[i]] = sec2[i]

        elif ob == 'boob':
            for i in range(0, len(sec)):
                global boob_dict
                boob_dict[sec[i]] = sec2[i]
        elif ob == 'cigarette':
            for i in range(0, len(sec)):
                global cigarette_dict
                cigarette_dict[sec[i]] = sec2[i]
        elif ob == 'knife':
            for i in range(0, len(sec)):
                global knife_dict
                knife_dict[sec[i]] = sec2[i]
        elif ob == 'gun':
            for i in range(0, len(sec)):
                global gun_dict
                gun_dict[sec[i]] = sec2[i]
        else:
            pass
    ...
    #obj_sec dict 구성 {'gun':[1,2,3,4,8], 'alcohol':[4,8,9]}
    # key 는 objname, value 는 초정보 list
    ...
    processing_time = round(stop - start, 2)

    # ratio 와 total ratio 에 없는 'obj':0.0 넣어주기
    class1=['alcohol', 'boob', 'cigarette', 'knife', 'gun']
    for cl in class1:
        if cl not in count_obj.keys():
            print(cl, "가 없다")
            count_obj[cl] = 0.0

        if cl not in total_obj.keys():
            print(cl, "가 없다")
            total_obj[cl] = 0.0
    ...
    #time dict 넣어주기
    for key in total_obj.keys():
        if key == 'alcohol':
            time_dict[key] = alcohol_dict
        elif key == 'boob':
            time_dict[key] = boob_dict
        elif key == 'gun':
            time_dict[key] = gun_dict
        elif key == 'cigarette':
            time_dict[key] = cigarette_dict
        elif key == 'knife':
            time_dict[key] = knife_dict
        else:
            pass
